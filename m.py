# -*- coding: utf-8 -*-
"""Min Hashing w/o hashes - IR.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1CakHTZCNgI9SCfQJPiVBQUXo_wJFz2KL
"""
...
def getPermutations(n):
  result = (random.sample(range(0,n),n))
  return result

from math import inf
def createSignatureMatrix(inputMatrix, permutations):
  sigMatrix = []
  r = len(permutations)
  c = len(inputMatrix[0])
  for i in range(r):
  ...
  for i,list1 in enumerate(permutations):
    for j,x in enumerate(list1):
      for k in range(c):
        if inputMatrix[x][k] != 0 and sigMatrix[i][k] > j+1:
          sigMatrix[i][k] = j+1
...
permutations = []
#No. of permutations = nop = 3
nop = 3
#Generate random permutations
for i in range(nop):
  permutations.append(getPermutations(len(inpMatrix)))
print(permutations)

#Find the Signature Matrix
sigMatrix = createSignatureMatrix(inpMatrix,permutations)
